...
export default function ChatScreen() {
    ...
    echo.private(channelName)
      .listen('.message.sent', (data: any) => {
        ...
        if (AppState.currentState !== 'active') {
          showLocalNotification(String(userName ?? 'New message'), data.message);
        }
      })
      .error((err: any) => console.error('Channel error:', JSON.stringify(err)));
  ...
  const sendMessage = async () => {
    ...
    const optimistic = {
      id: tempId, message: trimmed, sender_id: currentUserId,
      receiver_id: receiverId, created_at: new Date().toISOString(), temp: true,
    };
  ...
  const renderVoiceMessage = (item: any, isMe: boolean) => (
    <View style={[
      styles.voiceBubble,
      isMe ? styles.myVoiceBubble : [styles.theirVoiceBubble, { backgroundColor: colors.card }]
    ]}>
      ...
      <View style={styles.voiceMeta}>
        <Text style={[styles.voiceDuration, isMe ? styles.myVoiceText : { color: colors.subtext }]}>
          {item.duration || '0:30'}
        </Text>
        <Text style={[styles.timeText, isMe ? styles.myTimeText : { color: colors.icon }]}>
      ...
      <View style={[styles.messageRow, isMe ? styles.myMessageRow : styles.theirMessageRow]}>
        {isVoice ? renderVoiceMessage(item, isMe) : (
          <View style={[
            ...
            isMe ? styles.myMessage : [styles.theirMessage, { backgroundColor: colors.card }]
          ]}>
            <Text style={[
              styles.messageText,
      ...
      <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.header, {
          ...
          <View style={styles.headerInfo}>
            <Text style={[styles.headerName, { color: colors.text }]}>{userName || 'User'}</Text>
          ...
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: isDark ? colors.input : '#f4f3ff' }]}>
        ...
        >
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                <Ionicons name="chatbubbles-outline" size={56} color={colors.icon} />
                <Text style={[styles.emptyText, { color: colors.subtext }]}>No messages yet</Text>
                <Text style={[styles.emptySubtext, { color: colors.icon }]}>Say hello to start chatting!</Text>
              </View>
            }
          />
          ...
          <View style={[styles.inputBar, {
            ...
            <TouchableOpacity style={[styles.attachBtn, { backgroundColor: isDark ? colors.input : '#f4f3ff' }]}>
              <Ionicons name="attach" size={20} color={PURPLE} />
            </TouchableOpacity>
            <View style={[styles.inputWrap, {
              backgroundColor: isDark ? colors.input : '#f4f3ff',
              borderColor: isDark ? colors.border : '#e0dcff',
            }]}>
              <TextInput
...
const styles = StyleSheet.create({
  ...
  inputBar: {
    ...
    borderTopWidth: 1, gap: 6, marginBottom: 2,
  },
  attachBtn: {
    width: 34, height: 34, borderRadius: 17,
    alignItems: 'center', justifyContent: 'center',
  ...
  textInput: { fontSize: 13.5, maxHeight: 90, padding: 0 },
  sendBtn: {
  ...
  emptyText: { fontSize: 17, fontWeight: '500', marginTop: 14 },
